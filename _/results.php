...
<head>
    ...
    var gmb = calculaGMB($sexoo, $idadee, $massaa, $gestt, $NAFF);
    $('#cGMB').html(parseFloat(gmb.toFixed(2)));
    var GET = calculaGET(gmb, $sexoo, $idadee, $NAFF);
...
if ($sexoo == 1) { $sexoo = 'Masculino'; } else if ($sexoo == 2) { $sexoo = 'Feminino'; }
 if ($NAFF == 1) { $NAFF = 'Leve'; } else if ($NAFF == 2) { $NAFF = 'Moderada'; } else if ($NAFF == 3) { $NAFF = 'Intensa'; }
  if ($gestt == 1) { $gestt = 'block'; } else if ($gestt == 2) { $gestt = 'none'; }

    print("<script>
    $(function () {               
        $('#pnRNome').html('$nomee');
        $('#pnRIdade').html('$idadee' + ' anos');
        $('#pnRSexo').html('$sexoo');
        $('#pnRMassa').html('$massaa' + ' Kg');
        $('#pnREstatura').html('$estatt'+ ' m');
        $('#pnRNivel').html('$NAFF');
